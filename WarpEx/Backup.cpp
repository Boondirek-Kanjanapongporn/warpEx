...
void HOME_PAGE() {
	// DrawTextureEx(WarpEx_logo, logo_vector, 0.0f, 0.5f, WHITE);
	cout << "Home" << endl;
}
...
unique_ptr<ObjectImg> ReferenceImg, DisplayImg;
...
int main() {
	...
	if (fileCheck(path) == 0) {
		...
		// Preview Page 2
		DefaultImg = make_unique<ObjectImg>(path, "default");
		ResizeImg = make_unique<ObjectImg>(path, "resize");

		// Adjust Points Page 3
		// Display Preprocessing
		ResizeImg->setimgPreprocess(ResizeImg->preProcessing(ResizeImg->getimgOriginal()));
		...
		if (!(ResizeImg->getmainCont()).empty()) {
			ResizeImg->setmainCont(ResizeImg->reorderContourPoints());
			// Draw 4 Edges on image
			// drawPoints(DisplayImg->getmainCont(), Scalar(0,0,255), DisplayImg->getimgOriginal());

			// Document Page 4
			// Wrap image
			ResizeImg->setimgWarp(ResizeImg->processWarp(ResizeImg->getimgOriginal(), A4.x, A4.y));
			// Crop image
			ResizeImg->setimgWarp(ResizeImg->processCrop(ResizeImg->getimgWarp()));
			// Scan image
			ResizeImg->setimgScan(ResizeImg->processScan(ResizeImg->getimgWarp()));

			imshow("Img Crop", ResizeImg->getimgWarp());
			imshow("Img Scan", ResizeImg->getimgScan());

			// Extract Text
			/*char* extractText;
			tesseractExtract(ResizeImg->imgWarp, "eng", extractText);
			cout << extractText << endl;
			delete[] extractText;*/
		}
		imshow("image", ResizeImg->getimgOriginal());
	}
...
/*InitWindow(1000, 1000, "Test");
	...
	while (!WindowShouldClose()) {
		...
		if (first) {
			first = false;
			DisplayImage = LoadTexture("Resources/Temp/1619252168.png");;
		}
		DrawTexture(DisplayImage, 130, 130, WHITE);
		EndDrawing();
	}
	CloseWindow();*/
